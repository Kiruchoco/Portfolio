...
### 문제정의
 문해력은 문맹의 반의어로 사전적으로는 글을 읽을 줄 아는 것을 의미한다. 그렇지만 우리가 이야기하는 문해력은 정확히는 문해 능숙도를 의미하며 이는 ‘의미적 읽기’에 관한 역량 차이를 의미한다. 
 그렇다면 문해 능숙도의 차이는 어디에서 오는 것일까? 실제 EBS의 한 다큐멘터리에서는 그 차이가 익숙하지 않은 어휘에서 온다고 하였다. 즉, 어휘력이 풍부한 사람들은 글을 막힘 없이 읽을 수 있지만, 그렇지 않은 사람들은 글 읽기를 힘들어 한다. 그렇기에 우리는 문해력을 끌어올리기 위해서는 일정 수준 이상의 어휘력이 필요하다고 생각한다. 그렇지만 단순 암기를 통해 어휘력만 올려서 되는 것은 아니라고 생각한다. 어떤 문장에서 해당 어휘가 어떻게 사용되는 지를 보여주고 이를 활용해 보는 연어(連語)적 경험을 제시하는 것이 아이들의 문해력 향상을 위한 더 나은 해결책이라고 생각했다.
